...
export default function OrdersPage() {
  ...
  async function handleSelect(order: Order) {
    ...
    const { data } = await supabase
      .from('order_item')
      .select('*, product(name, code)')
      .eq('order_id', order.id)
    setItems(data || [])
  }

  async function handleStatusChange(orderId: string, status: string) {
    await supabase.from('order').update({ status, updated_at: new Date().toISOString() }).eq('id', orderId)
    loadOrders()
  ...
  return (
    <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
      <div style={{ flex: 1 }}>
        <h1 style={{ fontSize: '18px', fontWeight: '600', color: '#1e3a8a', marginBottom: '20px' }}>Siparişler</h1>
        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          {loading ? <p style={{ padding: '20px', fontSize: '13px', color: '#94a3b8' }}>Yükleniyor...</p> : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['Sipariş No', 'Bayi', 'Tarih', 'Tutar', 'Durum'].map(h => (
              ...
              <tbody>
                {orders.length === 0 && <tr><td colSpan={5} style={{ padding: '32px', textAlign: 'center', color: '#94a3b8' }}>Henüz sipariş yok</td></tr>}
                {orders.map((o, i) => (
                  <tr key={o.id} onClick={() => handleSelect(o)} style={{
                    ...
                    <td style={{ padding: '12px 16px', fontWeight: '500', color: '#1a2332' }}>{o.dealer?.name}</td>
      ...
      {selected && (
        <div style={{ width: '300px', flexShrink: 0 }}>
          ...
          <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
            ...
            <div style={{ marginBottom: '12px' }}>
              <p style={{ fontSize: '11px', color: '#94a3b8', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>Bayi</p>
              <p style={{ fontSize: '13px', fontWeight: '500', color: '#1a2332' }}>{selected.dealer?.name}</p>
            </div>
            <div style={{ marginBottom: '12px' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              <p style={{ fontSize: '11px', color: '#94a3b8', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '6px' }}>Durum</p>
              <select value={selected.status} onChange={e => handleStatusChange(selected.id, e.target.value)}
                style={{ width: '100%', border: '1.5px solid #e2e8f0', borderRadius: '8px', padding: '8px 10px', fontSize: '13px', color: '#1a2332', outline: 'none' }}>
            ...
            <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', marginBottom: '4px', color: '#64748b' }}>
                <span>Ara toplam</span><span>TL{Number(selected.subtotal).toFixed(2)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', marginBottom: '8px', color: '#64748b' }}>
                <span>KDV</span><span>TL{Number(selected.vat_amount).toFixed(2)}</span>
              </div>
